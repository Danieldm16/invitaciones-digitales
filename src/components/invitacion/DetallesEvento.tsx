...
// Paleta por defecto elegante si el evento no especifica una
...
export const ItinerarioSeccion = ({ 
  ...
  itinerario?: { h: string; a: string; icon?: string }[], 
  lugar?: string, 
  mapa?: string, 
  esOscuro?: boolean 
}) => {
              ...
              <div className="space-y-2">
                ...
                {mapa && index === 0 && (
                  <a 
                    ...
                    className={`inline-flex items-center gap-1.5 mt-2 px-6 py-1.5 rounded-full border ${esOscuro ? "border-[#D4AF37] text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black" : "border-[#C5A880] text-[#8A6F48] hover:bg-[#C5A880]/10"} text-[11px] uppercase tracking-[0.2em] transition-colors font-sans`}
                  >
                    Ver Mapa ↗
                  </a>
                )}
  ...
  return (
    <section className="px-6 py-4 space-y-6">
      <div className={`${esOscuro ? "bg-[#181614] border-[#C5A880]/30" : "bg-white border-[#EBE4D8]"} rounded-3xl p-6 shadow-md border space-y-5`}>
        ...
        {/* Advertencia / Nota: SOLO se muestra si el usuario escribió algo en el panel */}
